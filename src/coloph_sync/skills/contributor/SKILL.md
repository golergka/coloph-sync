...
Use `sync-merge-main` when the current branch needs the configured local main branch, or when the coordinator reports a merge conflict for the current tip. Read both histories from their shared parent. Preserve both intended behaviors. Use a real merge. Do not rebase or recreate selected changes.
